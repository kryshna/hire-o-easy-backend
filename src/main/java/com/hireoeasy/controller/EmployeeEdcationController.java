...
public class EmployeeEdcationController {
	...
	// Get education
	// @GetMapping(value =
	// "/webapi/employee/employeedetail/education/{user_detail_id}")
	// public ResponseEntity<List<EmployeeEducation>>
	// getAllJobs(@PathVariable("user_detail_id") Long id) {
	...
	public ResponseEntity<List<EmployeeEducation>> getAllJobs(@PathVariable("user_detail_id") Long id) {
		...
		List<EmployeeEducation> educationList = new ArrayList<>();
		if (userDetail.isPresent()) {
			educationList = userDetail.get().getEmployeeEducation();
		}
		return ResponseEntity.ok().body(educationList);
	}
	...
	public ResponseEntity<?> postEducation(@RequestBody EmployeeEducation employeeEducation,
